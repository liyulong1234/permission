	    ...
	    <div class="row">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form class="form-horizontal" action="save-dept.do" method="post">
                    ...
                    <div class="ibox-content">                    	
                        ...
                        <div class="row">
                            <div class="table-responsive">
                                <form id="deleteManyForm" action="deleteMany.do" method="post">
                                <table class="table table-striped list-table">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${pageInfo.list}" var="dept">
                                            <tr>
                                                <td><input name="ids" type="checkbox" value="${dept.deptno}"></td>
                                                <td>${dept.deptno}</td>
                                                <td>${dept.dname}</td>
                                                <td>${dept.local}	</td>
                                                <td>
                                                 <a href="update-dept.do?deptno=${dept.deptno}"><i class="glyphicon glyphicon-edit  text-navy"></i></a>
                                                 <a href="javascript:deleteOne(${dept.deptno})" class="btndel"><i class="glyphicon glyphicon-remove  text-navy"></i></a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                        ...
                        <div class="row">
							...
							<div class="col-sm-7 text-right">
								<div class="btn-group">
	                                ...
	                                <a href='?currentPage=${pageInfo.pageNum - 1}'>上一页</a>
                                    [
                                    <c:forEach items="${pageInfo.navigatepageNums}" var="num">
                                        <c:choose>
                                            <c:when test="${pageInfo.pageNum == num}">
                                                &nbsp;<a href='?currentPage=${num}'><span style="color: red;">${num}</span></a>&nbsp;
                                            </c:when>
                                            <c:otherwise>
	                                ...
	                                <a href='?currentPage=${pageInfo.pageNum + 1}'>下一页</a>
	                                <a href='?currentPage=${pageInfo.pages}'>尾页</a>
	                            </div>
							</div>
                        </div>
                        </div>
    ...
    <script type="text/javascript">
        ...
        function deleteOne(id) {
            swal({
                ...
                //同步 -- 等同于超链接
                location.href = "deleteOne.do?deptno="+id;
            });
        }
        ...
        function deleteMany() {
            //是否选中了选项
            var cs = $("input[type='checkbox']:checked");//只获取被选中的复选框
            if(cs.length > 0){
                swal({
                    title: "您确定要删除这些信息吗",
                    text: "删除后将无法恢复，请谨慎操作！",
